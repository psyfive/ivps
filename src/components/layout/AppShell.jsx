...
export function AppShell() {
  const { screen, selectedSkill, skill, practiceFullscreen } = usePractice();

  return (
    <div className="flex h-screen w-screen overflow-hidden ivps-bg text-ivps-text">
      {/* 좌측 네비게이션 사이드바 — 연습 전체화면 시 숨김 */}
      {!practiceFullscreen && <LeftNav />}
      ...
      {/* 우측 유틸리티 패널 (Cockpit 화면에서만, 연습 전체화면 시 숨김) */}
      {screen === 'cockpit' && !practiceFullscreen && <RightUtilPanel />}
