    ...
    <button
      ...
      className={[
        ...
        checked
          ...
          : 'border-[#1a2035] bg-[#131720] hover:border-[rgba(155,127,200,.2)] hover:bg-[rgba(155,127,200,.05)]',
      ].join(' ')}
    >
      {/* 번호 / 체크 */}
      <span
        className={[
          ...
          checked
            ...
            : isFirst
            ? 'bg-[rgba(155,127,200,.22)] text-[#9b7fc8]'
            : 'bg-[#1a2035] text-[#4a5568]',
        ].join(' ')}
      >
        {checked ? '✓' : index + 1}
      </span>
      ...
      {/* 핵심 배지 */}
      {isFirst && !checked && (
        <span className="text-[9.5px] font-mono text-[#9b7fc8] bg-[rgba(155,127,200,.15)] border border-[rgba(155,127,200,.25)] px-1.5 py-0.5 rounded flex-shrink-0">
          핵심
        </span>
      )}
    </button>
...
function AdaptiveTip({ streak, bpm }) {
  ...
  return (
    <div className="flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-[rgba(212,168,67,.2)] bg-[rgba(212,168,67,.06)] mb-3">
      <span className="text-[12px] text-[#d4a843]">{messages[0]}</span>
      <span className="text-[#d4a843] text-[12px]">›</span>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-5 pt-4 pb-3 flex-shrink-0">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-1.5">
            {streak > 0 && (
              <span className="font-mono text-[10px] text-[#d4a843] bg-[rgba(212,168,67,.1)] border border-[rgba(212,168,67,.2)] px-1.5 py-0.5 rounded">
                🔥 ×{streak}
              </span>
            )}
            <button
              onClick={resetChecks}
              className="text-[10px] text-[#3d4455] hover:text-[#4a5568] font-mono transition-colors"
            >
              리셋
            </button>
          </div>
        ...
        {/* 세션 컨텍스트 안내 */}
        {sessionSkill && (
          <div className="text-[10.5px] text-[#3d4455] mt-1">
      ...
      <div className="px-5 mb-3 flex-shrink-0">
        <div className="text-[10px] text-[#4a5568] uppercase tracking-[.08em] font-semibold flex items-center gap-1.5">
          <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#9b7fc8]" />
          HUD · 찰나의 체크포인트
          <span className="ml-auto font-mono text-[#9b7fc8]">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-3">
        ...
        >
          <div className="text-[10.5px] text-[#4a5568] uppercase tracking-[.07em] font-semibold mb-3">
            연습 회차 완료
          </div>
        ...
        {activeSession?.skills?.length > 1 && (
          <div className="mt-4">
            <div className="text-[10px] text-[#3d4455] uppercase tracking-[.08em] font-mono mb-2">
              이 세션의 다른 스킬
            </div>
            <div className="flex flex-col gap-1.5">
              {activeSession.skills.slice(1).map(id => {
                ...
                return (
                  <div
                    key={id}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#0d1117] border border-[#1a2035]"
                  >
                    <span
                    ...
                    <span className="text-[12px] text-[#8896ae]">{sk.name}</span>
                  </div>
                );
              })}
            </div>
